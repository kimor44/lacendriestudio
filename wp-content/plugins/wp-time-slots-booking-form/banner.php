...
if(!function_exists( 'codepeople_add_promote_banner' ))
{
	function codepeople_add_promote_banner($wp_admin_bar)
	{
		...
		{
            $cpkey = sanitize_key($_POST['codepeople_promote_banner_plugin']);
			if(
				!empty($_POST['codepeople_promote_banner_plugin']) &&
				!empty($codepeople_promote_banner_plugins[$cpkey])
			)
			{
				set_transient( 'codepeople_promote_banner_'.$cpkey, -1, 0);
				if(
					!empty($_POST['codepeople_promote_banner_action']) &&
					$_POST['codepeople_promote_banner_action'] == 'set-review' &&
					!empty($codepeople_promote_banner_plugins[$cpkey]['plugin_url'])
				)
				{
					print '<script>document.location.href="'.esc_js($codepeople_promote_banner_plugins[$cpkey]['plugin_url']).'";</script>';
				}
			}
		}
				...
				<div id="codepeople-review-banner">
					...
					<div class="codepeople-review-banner-content">
						<div class="codepeople-review-banner-text">
							<p>Hey, I noticed you are using the "<?php print esc_html($plugin_data[ 'plugin_name' ]); ?>" plugin – <strong>that’s awesome!</strong> Could you please do me a <strong>BIG</strong> favor and <a href="<?php echo esc_url($plugin_data["plugin_url"]); ?>" style="color:#1582AB;font-weight:bold;text-decoration: underline;">give it a 5-star rating on WordPress</a>? Just to help us spread the word and boost our motivation. <strong>Thank you!</strong></p>                            
						</div>
