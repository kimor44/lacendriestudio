...
class Renderer {
    ...
    public function render_table_row( $row, $table ) {
        ...
        $result = apply_filters( 'plugion_table_row', $this->load_template( 'table_row', [ $row, $table ] ), $row, $table );
        echo $result;
    }
    public function load_template( $template, $data ) {
        $file_name = PLUGION_DIR . DIRECTORY_SEPARATOR . 'templates' . DIRECTORY_SEPARATOR . $template . '.php';
