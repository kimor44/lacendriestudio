...
class WBK_Backend_Schedule  {
	public function __construct() {
		...
		add_action( 'wp_ajax_wbk_schedule_load_fullcalendar', array( $this, 'schedule_load_fullcalendar' ) );
    ...
    public function wbk_create_multiple_bookings(){ 
		...
		$html = __( 'Appointments added:', 'webba-booking-lite' ) . ' ' . count( $appointment_ids );
		date_default_timezone_set( 'UTC' );
		echo $html;
		wp_die();
		return;
	...
	public function wbk_delete_appointment_fullcalendar() {
		...
		foreach ( $time_slots as $key => $time_slot ) {
			if( !($time_slot->start == (int)$time) ) continue;
			$html_schedule = WBK_Renderer::load_template( 'backend/schedule_day_timeslot', array( $time_slot, $service_id, $sp->get_locked_time_slots( $service_id ) ), false );
		}
	...
	public function view_appointment_fullcalendar() {
		...
		$time_format = WBK_Date_Time_Utils::get_time_format();
		$time_string = wp_date( $date_format, $time, new DateTimeZone( date_default_timezone_get() ) ) . ' ' . wp_date( $time_format, $time, new DateTimeZone( date_default_timezone_get() ) );
		$resarray = array( 'name' => $name, 'desc' =>  $desc, 'email' => $email, 'phone' => $phone, 'time' => $time_string, 'timestamp' => $time, 'extra' => $extra, 'quantity' => $quantity );
		echo json_encode($resarray);
		date_default_timezone_set( 'UTC' );
		die();
		return;
	...
	public function add_appointment_backend_fullcalendar() {
		...
		$sp = new WBK_Schedule_Processor();
		if( $time == $edited_time ) {
			$response['day'] = $this->get_timeslot($sp, $time, $service_id, $is_new);
		} else {
			$response['day'] = $this->get_timeslot($sp, $time, $service_id, $is_new);
			$response['edited_day'] = $this->get_timeslot($sp, $edited_time, $service_id);
		}
	...
	public function get_timeslot($sp, $time, $service_id, $is_new = false) {
		...
		foreach ( $time_slots as $key => $time_slot ) {
			if( !($time_slot->start == (int)$time) ) continue;
			$html_schedule = WBK_Renderer::load_template( 'backend/schedule_day_timeslot', array( $time_slot, $service_id, $sp->get_locked_time_slots( $service_id ), $is_new ), false );
		}
	...
	public function schedule_load_fullcalendar() {
		...
		if ( $start > 0 ) {
			...
			if($response['offset'] < 0) {
				$timestamp = $start_date->getTimestamp() + abs($response['offset']);
			}
		...
		if ( $initial_view == 'dayGridMonth' ) {
			...
			$day_to_render = $date->getTimestamp();
		} else if ( $initial_view == 'timeGridWeek' ) {			
		...
		if ( $is_multiple_services ) {
			foreach ( $service_ids as $service_id ) {
				...
				$bookings_ids = WBK_Model_Utils::get_booking_ids_by_range_service($response['first_day'], $response['last_day'], $service_id);
				...
				foreach ( $bookings_ids as $booking_id ) {
					...
					$temp =  WBK_Time_Math_Utils::adjust_times( $booking->get_local_time(), $total_duration, get_option( 'wbk_timezone', 'UTC' ) );

					$time_slot = new WBK_Time_Slot( $booking->get('time'), $temp );
                    $time_slot->end = $time_slot->start + $booking->get('duration') * 60;
					$time_slot->display = 'block';
					$time_slot->set_free_places( 0 );
					$time_slot->set_status( $booking_id );
					
					$html_schedule = WBK_Renderer::load_template( 'backend/schedule_day_timeslot', array( $time_slot, $service_id, [] ), false );

					$color = "rgba( $colors[$service_id], 0.3)";
					$background_color = "background-color: $color;";
					$time_slot->backgroundColor = $color;

					$html_schedule =  str_replace('class="timeslot_container"', 'class="timeslot_container" style="' . $background_color . '"', $html_schedule);
					$time_slot->html = $html_schedule;
					
                    $temp_start = $time_slot->start;
                    $time_slot->start = date('Y-m-d H:i:s', $time_slot->start);
                    $time_slot->end = $temp_start + $booking->get('duration') * 60;
                    $time_slot->end = date('Y-m-d H:i:s', $time_slot->end);
                   
					$response['events'][$day_to_render]['time_slots'][] = $time_slot;
				}
			}
		}
	...
	// ajax unlock day
 	...
 	public function ajax_unlock_time() {
        ...
        echo '<a id="app_add_' . esc_attr( $service_id . '_' . $time ) . '"><span class="dashicons dashicons-welcome-add-page"></span></a>';
