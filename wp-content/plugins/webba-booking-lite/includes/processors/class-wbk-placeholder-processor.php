...
class WBK_Placeholder_Processor {
    ...
    public static function process_placeholders( $message, $bookings ){
        ...
        if( is_numeric( $bookings ) ){
            ...
            $payment_details = WBK_Price_Processor::get_payment_items( array( $bookings ), get_option( 'wbk_general_tax', '0'), null, false );
            $message = self::message_placeholder_processing_old( $message, $booking->get_id(), $booking->get_service(), $payment_details );
            ...
            if( WBK_Validator::check_email_loop( $message ) ){
                ...
                foreach ( $bookings as $booking_id ){
                    ...
                    $looped_html .= self::message_placeholder_processing_old( $looped, $booking_id, $booking->get_service() );
            ...
            $payment_details = WBK_Price_Processor::get_payment_items( $bookings, get_option( 'wbk_general_tax', '0'), null, false );
            $message = self::message_placeholder_processing_old( $message, $booking_id, $booking->get_service(), $payment_details, $token, $admin_token );
        }
    ...
    public static function process_not_booked_item_placeholders( $service_ids, $times, $category_id ){
        ...
        if( count( $form_label_initial ) == 2){
            ...
            /*fix */
            $html = str_replace( '#total_amount', '<span class="wbk_form_label_total"></span>', $html );
        ...
        foreach( $service_ids as $service_id ){
            ...
            $price_format = get_option( 'wbk_payment_price_format', '$#price' );
            $price = str_replace( '#price', number_format( $service->get_price(),  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
            ...
            foreach( $times_this as $time ) {
                ...
                $date_collect[] = esc_html( wp_date( $date_format, $time, new DateTimeZone( date_default_timezone_get() ) ) );
                $time_collect[] = esc_html(  wp_date( $time_format, $time, new DateTimeZone( date_default_timezone_get() ) ) );
                $time_local_collect[] = esc_html(  wp_date( $time_format, $time + $current_offset, new DateTimeZone( date_default_timezone_get() ) ) );
                $date_local_collect[] = esc_html(  wp_date( $date_format, $time + $current_offset, new DateTimeZone( date_default_timezone_get() ) ) );
                $datetime_collect[] = esc_html( wp_date( $date_format, $time, new DateTimeZone( date_default_timezone_get() ) ) ) . ' ' .  esc_html( wp_date( $time_format, $time, new DateTimeZone( date_default_timezone_get() ) ) );
                $datetime_local_collect[] = wp_date( $date_format, $time + $current_offset, new DateTimeZone( date_default_timezone_get() ) ) . ' ' . esc_html( wp_date( $time_format, $time + $current_offset, new DateTimeZone( date_default_timezone_get() ) ) );
                $datetime_n_collect[] = '<br>' . esc_html( wp_date( $date_format, $time, new DateTimeZone( date_default_timezone_get() ) ) ) . ' ' . esc_html( wp_date( $time_format, $time, new DateTimeZone( date_default_timezone_get() ) ) );
                $datetimerange_n_collect[] = '<br>' . esc_html( wp_date( $date_format, $time, new DateTimeZone( date_default_timezone_get() ) ) ) . '   ' . esc_html( wp_date( $time_format, $time, new DateTimeZone( date_default_timezone_get() ) ) ) . ' - ' . esc_html( wp_date( $time_format, $end_this, new DateTimeZone( date_default_timezone_get() ) ) );
                $date_n_collect[] = '<br>' . esc_html( wp_date( $date_format, $time, new DateTimeZone( date_default_timezone_get() ) ) );

            }

            $time_range = esc_html( wp_date( $time_format, $start, new DateTimeZone( date_default_timezone_get() ) ) ) . ' - ' . esc_html( wp_date( $time_format, $end, new DateTimeZone( date_default_timezone_get() ) ) );
            $local_time_range = esc_html( wp_date( $time_format, $start_local, new DateTimeZone( date_default_timezone_get() ) ) ) . ' - ' . esc_html( wp_date( $time_format, $end_local, new DateTimeZone( date_default_timezone_get() ) ) );
            $single_start_date =  esc_html( wp_date( $date_format, $start, new DateTimeZone( date_default_timezone_get() ) ) );
    ...
    public static function message_placeholder_processing_old( $message, $booking_id, $service_id, $payment_details = null, $multi_token = null, $multi_token_admin = null ){
        $booking = new WBK_Booking( $booking_id );
        if( !$booking->is_loaded() ){
        ...
        $timezone_to_use = WBK_Date_Time_Utils::convert_default_time_zone_to_utc( $booking->get_start() );
		...
		if( $payment_link_url != '' ){
			...
			if( $token != false ){
				if( $payment_token != '' ){
					$payment_link = '<a target="_blank" target="_blank" href="' . esc_url( $payment_link_url ) . '?order_payment=' . esc_html( $payment_token ) . '">' . trim( esc_html( $payment_link_text ) ) . '</a>';
				} else {
					$payment_link = '';
				}
			    $cancel_link = '<a target="_blank" target="_blank" href="' . esc_url( $payment_link_url )  . '?cancelation=' . esc_html( $token ) . '">' . trim( esc_html( $cancel_link_text ) ) . '</a>';
			    $gg_add_link = '<a target="_blank" target="_blank" href="' . esc_url( $payment_link_url )  . '?ggeventadd=' . esc_html( $token ) . '">' . trim( esc_html( $gg_add_link_text ) ) . '</a>';
			 }
		}
		...
		if( get_option( 'wbk_allow_manage_by_link', 'no' ) == 'yes' ){
			if( $payment_link_url != '' ){
				...
				if( $token != false ){
					$admin_cancel_link = '<a target="_blank" target="_blank" href="' . esc_url( $payment_link_url ) . '?admin_cancel=' . esc_html( $token ) . '">' . trim( esc_html( $admin_cancel_link_text ) ) . '</a>';
				    $admin_approve_link = '<a target="_blank" target="_blank" href="' . esc_url( $payment_link_url ). '?admin_approve=' . esc_html( $token ) . '">' . trim( esc_html( $admin_approve_link_text ) ) . '</a>';
				}
			}
		}
		...
		if( is_numeric( $coupon_id ) && $coupon_id > 0 ){
            ...
            if( !$coupon->is_loaded() ){
                $discount_data =array( $coupon->get('amount_fixed'), $coupon->get('amount_percentage') );
		...
		if( get_option( 'wbk_allow_attachemnt', 'no' ) == 'yes' ){
			...
			if( $attachment !== '' ){
				...
				if( is_array( $attachment ) ){
					...
					$attachment = '<a rel="noopener" target="_blank" href="' . esc_url( $attachment ) . '">' . esc_html( $attachment ) . '</a>';
				}
			}
		}
		...
		$message = str_replace( '#service_description', $service->get('description'), $message );
		...
		$message = str_replace( '#appointment_day', wp_date( $date_format, $booking->get_day() + $correction, $timezone_to_use ), $message );
		$message = str_replace( '#appointment_time', wp_date( $time_format,  $booking_start , $timezone_to_use ), $message ) ;
         
		$message = str_replace( '#appointment_local_time', wp_date( $time_format, $booking->get_local_time(), $timezone_to_use  ), $message );
		$message = str_replace( '#appointment_local_date', wp_date( $date_format, $booking->get_local_time(), $timezone_to_use  ), $message );
		...
		$time_range = wp_date( $time_format, $booking->get_start() + $correction,  $timezone_to_use ) . ' - ' . wp_date( $time_format, $booking->get_start() + $correction + $service->get_duration() * 60, $timezone_to_use );
		...
		$one_slot_price = str_replace( '#price', number_format( $service->get_price(),  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
		...
		$moment_price = str_replace( '#price', number_format( $moment_price,  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
