...
function slf_table_render_add_form(class_name) {
    ...
    jQuery.post(ajaxurl, data, function (response) {
        jQuery('#slf_table_add_panel').fadeOut('fast', function () {
            ...
            if (jQuery('#wbk_default_app_status').length > 0) {
                jQuery(".slf_table_component_select[name='status']").val(
                    jQuery('#wbk_default_app_status').val()
                )
            }
            // add row start
            jQuery('#' + col_form_id)
                .find('.slf_table_row_save')
                .click(function () {
                    var fields = []
                    jQuery(this)
                        .parent()
                        .parent()
                        .find('.slf_table_component_input')
                        .each(function () {
                            var component_name = jQuery(this).attr('name')
                            var component_val
                            if (
                                jQuery(this).hasClass(
                                    'slf_table_component_editor'
                                )
                            ) {
                                component_val = tinyMCE
                    ...
                    jQuery('#' + col_form_id)
                        ...
                        .prop('disabled', true)
                    jQuery('#' + col_form_id)
                        .find('.slf_table_row_cancel')
                        .prop('disabled', true)
                    jQuery('#' + col_form_id)
                        .find('.slf_control_error_message')
                    ...
                    jQuery.post(ajaxurl, data, function (response) {
                        ...
                        if (response.status == 1) {
                            jQuery('#slf_table_add_panel').fadeOut(
                                'fast',
                                function () {
                                    jQuery('.slf-table > tbody').append(
                                        response.data
                                    )
                                    jQuery('#slf_table_add_panel').html(
                                        create_button_html
                                    )
                                    jQuery('#slf_table_add_panel').fadeIn(
                                        'fast',
                                        function () {
                                            if (
                                                typeof slf_row_after_add ===
                                                'function'
                                            ) {
                                                slf_row_after_edit()
                                            }
...
function slf_table_prepare_delete_row(id, class_name) {
    ...
    var delete_panel =
        '<div class="slf_control_container"><input type="button" class="button-primary slf_table_row_delete" value="Delete" /><input type="button" class="button-primary slf_table_row_cancel" value="cancel" /></div><div class="slf_control_error_message"></div>'
    jQuery(
        '<tr style="display:none;background:rgb(243, 243, 243);" id="' +
            row_form_id +
            '"><td id="' +
            col_form_id +
            '" colspan="' +
            ...
            jQuery.post(ajaxurl, data, function (response) {
                ...
                if (response == '1') {
                    jQuery('#' + row_form_id).fadeOut('fast', function () {
                        ...
                        if (jQuery('[id^=row-form-]').length == 0) {
                            jQuery('.tablesaw-sortable-btn').prop(
                                'disabled',
                                false
                            )
                        }
            ...
            .click(function () {
                ...
                jQuery(this)
                    ...
                    .each(function () {
                        ...
                        var component_val
                        if (
                            jQuery(this).hasClass('slf_table_component_editor')
                        ) {
                            component_val = tinyMCE
                                .get(jQuery(this).attr('id'))
                                .getContent()
...
function slf_set_events() {
    ...
    jQuery('.slf_date_range_start, .slf_date_range_end')
        .datepicker({ dateFormat: wbk_filter_date_format })
        .on('change', function () {
            jQuery('.slf_date_range').val(
                jQuery('.slf_date_range_start').val() +
                    ';' +
                    jQuery('.slf_date_range_end').val()
            )
            jQuery('.slf_date_range').trigger('change')
        })
    jQuery('#wbk_filter_services_control')
        .chosen({ width: '95%' })
        .change(function () {})
    // filter change event
    ...
    jQuery('.slf_table_csv_export').click(function () {
        if (jQuery('.slf-table').find('tr').length < 2) {
            return
        }
        var filters = []
        jQuery('.slf_filter').each(function () {
        ...
        var class_name = jQuery('#slf_table_class_name').val()
        var data = {
            action: 'slf_table_export_csv',
            filters: filters,
            class_name: class_name,
        }
        jQuery('#slf_table_csv_export_container').html(
    ...
    jQuery('.slf-component').change(function () {
        ...
        var special = false
        if (
            class_name == 'wbk-checkbox:after' ||
            class_name == 'wbk-checkbox' ||
            class_name ==
                'wbk-checkbox + label::before, .wbk-checkbox + span::before'
        ) {
            jQuery(
                '<style> .wbk-checkbox + label::before, .wbk-checkbox + span::before{' +
                    prop_name +
                    ':' +
                    value +
                    ' !important;}</style>'
    ...
    jQuery('.slf-type-pm-sub').on('input', function () {
        ...
        if (!slf_check_css_length(val)) {
            jQuery(this).addClass('slf-input-error-wb', 500, 'easeOutQuad')
        } else {
            jQuery(this).removeClass('slf-input-error-wb')
    ...
    jQuery('.slf-type-border-sub').on('input', function () {
        ...
        if (jQuery(this).hasClass('slf-type-border-color')) {
            if (color == '') {
                jQuery(this).addClass('slf-input-error-wb')
            } else {
                jQuery(this).removeClass('slf-input-error-wb')
            }
        }
    ...
    // trigger table update if services selected
    if (jQuery('#wbk_filter_services_control option:selected').length > 0) {
        jQuery('#wbk_filter_services_control').trigger('change')
    }
}

// save data
function slf_save_sections_set(framework_slug, section_set_slug) {
    var error_count = jQuery('.slf-input-error-wb').size()
    if (error_count > 0) {
        jQuery('.slf_notice').html(
            error_count + ' validation errors, please fix it.'
        )
