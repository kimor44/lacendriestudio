...
class WBK_Email_Notifications
{
    ...
    public function send( $event, $send_single = FALSE )
    {
        ...
        switch ( $event ) {
            case 'book':
                ...
                if ( $this->customer_book_status != '' ) {
                    ...
                    if ( $send_single == TRUE || get_option( 'wbk_multi_booking', 'disabled' ) != 'disabled' && get_option( 'wbk_email_customer_book_multiple_mode', 'foreach' ) == 'foreach' || get_option( 'wbk_multi_booking', 'disabled' ) == 'disabled' ) {
                        //	validation
                        if ( !WBK_Validator::checkStringSize( $this->customer_email_message, 1, 50000 ) || !WBK_Validator::checkStringSize( $this->customer_email_subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 200 ) ) {
                            return;
                        }
                ...
                if ( $this->admin_book_status != '' ) {
                    ...
                    if ( $send_single == TRUE || get_option( 'wbk_multi_booking', 'disabled' ) != 'disabled' && get_option( 'wbk_email_admin_book_multiple_mode', 'foreach' ) == 'foreach' || get_option( 'wbk_multi_booking', 'disabled' ) == 'disabled' ) {
                        //	validation
                        if ( !WBK_Validator::checkStringSize( $this->admin_email_message, 1, 50000 ) || !WBK_Validator::checkStringSize( $this->admin_email_subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 200 ) ) {
                            return;
                        }
            ...
            case 'daily':
                //	validation
                if ( !WBK_Validator::checkStringSize( $this->admin_daily_message, 1, 50000 ) || !WBK_Validator::checkStringSize( $this->admin_daily_subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 100 ) ) {
                    $this->admin_daily_status = false;
                }
                if ( !WBK_Validator::checkStringSize( $this->customer_daily_message, 1, 50000 ) || !WBK_Validator::checkStringSize( $this->customer_daily_subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 100 ) ) {
    ...
    public function sendMultipleToSecondary( $appointment_ids, $data )
    {
        ...
        if ( !WBK_Validator::checkStringSize( $this->secondary_email_message, 1, 50000 ) || !WBK_Validator::checkStringSize( $this->secondary_email_subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 200 ) ) {
            return;
        }
    ...
    public function sendOnApprove()
    {
        ...
        if ( $this->customer_approve_status != '' ) {
            ...
            if ( !WBK_Validator::checkStringSize( $this->customer_approve_message, 1, 50000 ) || !WBK_Validator::checkStringSize( $this->customer_approve_subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 200 ) ) {
                date_default_timezone_set( 'UTC' );
                return;
            }
    ...
    public function prepareOnCancel()
    {
        ...
        if ( $this->admin_cancel_status != '' ) {
            ...
            //	validation
            if ( !WBK_Validator::checkStringSize( $this->admin_cancel_message, 1, 50000 ) || !WBK_Validator::checkStringSize( $this->admin_cancel_subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 200 ) ) {
                return;
            }
    ...
    public function prepareOnCancelCustomer( $by_customer = false )
    {
        ...
        if ( $this->customer_cancel_status != '' ) {
            ...
            //	validation
            if ( !WBK_Validator::checkStringSize( $this->customer_cancel_message, 1, 50000 ) || !WBK_Validator::checkStringSize( $this->customer_cancel_subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 200 ) ) {
                return;
            }
    ...
    public function sendSingleInvoice()
    {
        ...
        //	validation
        if ( !WBK_Validator::checkStringSize( $message, 1, 50000 ) || !WBK_Validator::checkStringSize( $this->customer_invoice_subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 200 ) ) {
            return;
        }
        $message = $this->message_placeholder_processing( $message, $appointment, $service );
    ...
    public function sendSinglePaymentReceived( $to )
    {
        ...
        //	validation
        if ( !WBK_Validator::checkStringSize( $message, 1, 50000 ) || !WBK_Validator::checkStringSize( $subject, 1, 200 ) || !WBK_Validator::checkEmail( $this->from_email ) || !WBK_Validator::checkStringSize( $this->from_name, 1, 200 ) ) {
            return;
        }
        $message = $this->message_placeholder_processing( $message, $appointment, $service );
