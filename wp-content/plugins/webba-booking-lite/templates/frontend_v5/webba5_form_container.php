    ...
    <div class="appointment-box-w">             
        ...
        ?>
                <div class="appointment-status-wrapper-w">
                    ...
                    <div class="circle-chart-wb" data-circle-color="<?php echo esc_attr( $circle_color ); ?>" data-percent="25"><span class="circle-chart-text-wb"></span></div>
        ...
        <div class="appointment-content-w">            
            ...
            <form>
                <div class="appointment-content-scroll-w" data-scrollbar="true" tabindex="-1" style="overflow: hidden; outline: none;">
                        ...
                        <?php
                            ...
                            foreach( $scenario as $screen ){
                                ...
                                if( $i == 0 ){
                                    $class .= ' appointment-content-screen-active-w '; 
                                } 
                                $title = '';
                        ...
                        ?>
                            <div class=" <?php echo esc_attr( $class ); ?>"  <?php echo $final_request_attr; ?>  data-slug="<?php echo esc_attr( $screen['slug'] ); ?>"  data-title="<?php echo $title; ?>"  data-request="<?php echo esc_attr( $screen['request'] );  ?>">
                            <?php
                                if( isset(  $screen['templates']  ) &&   is_array( $screen['templates'] ) ){
                                    foreach( $screen['templates'] as $template => $args ){
                                        WBK_Renderer::load_template( $template, $args );
                                    }
                                }
                            ?>                             
                            </div>
                ...
                <div class="button-block-w two-buttons-w">
                    <button type="button" class="button-w button-prev-w"><?php echo esc_html( get_option( 'wbk_back_button_text', __( 'Back'  ) ) ); ?></button>
                    <button type="button" class="button-w button-next-w"><?php echo esc_html( get_option( 'wbk_next_button_text', __( 'Next' ) ) ) ?><span class="btn-ring-wb"></span></button>
                </div><!-- /.button-block-w -->
