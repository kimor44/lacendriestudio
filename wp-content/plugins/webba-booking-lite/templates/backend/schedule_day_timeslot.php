...
if ( $timeslot->get_status() == -2 || ( in_array( $timeslot->get_start(), $locked_time_slots ) && !is_array( $timeslot->get_status() )  ) ) {
    ...
    foreach ( $booking_ids as $booking_id ) {
        ...
        $time_controls .= '<a class="wbk-appointment-backend" id="wbk_appointment_' . esc_attr( $booking_id ) . '_'. esc_attr( $service_id ) .'_1" >' . esc_html( str_replace( '&#039;',"'", stripslashes( $booking->get_name() ) ) ) . '</a>';
    }
...
if( $timeslot->get_status() != 0 ) {
    ...
    if($booking_ids) {
        foreach ( $booking_ids as $booking_id ) {
            ...
            $time_range =  wp_date( get_option('time_format'), $booking->get_start(), new DateTimeZone( date_default_timezone_get() ) ) . '   ';
            // $time_range .= ' - ' .  wp_date( get_option('time_format'), $booking->get_end(), new DateTimeZone( date_default_timezone_get() ) ) . ' ';
          
            
            $timeslot->title = $time_range .  str_replace( '&#039;',"'", stripslashes( $booking->get_name() ) );
            ...
            if ($is_new || count($booking_ids) > 1) {
                $html_schedule .= '<div class="fc-event-main-frame">
                                <div class="fc-event-title-container">
                                    <div class="fc-event-title fc-sticky">' . $time_range  .  esc_html( str_replace( '&#039;',"'", stripslashes( $booking->get_name() ) )  ) . '</div>
                                </div>
                            </div>';
            }
            ...
            $html_schedule .= '<div class="event-popover-wb">
                                    <div class="popover-title-wb">' . esc_html( str_replace( '&#039;',"'", stripslashes( $booking->get_name() ) ) . $quantity ) . '</div>
                                    <div class="popover-time-wb">' . date( get_option('time_format'), $timeslot->start ) . '-' . date( get_option('time_format'), $timeslot->end ) . '</div>
                                    <div class="popover-time-wb">'. $booking->get('description') . '</div>
    ...
    $html_schedule .= '<div class="timeslot_container" data-timeslot-timestamp="'.$timeslot->start.'">
