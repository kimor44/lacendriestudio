...
if( get_option( 'wbk_allow_coupons', 'disabled' ) == 'enabled' ){
    $payment_init_html .= '<input class="wbk-input" id="wbk-coupon" placeholder="' . esc_attr( WBK_Validator::alfa_numeric( get_option( 'wbk_coupon_field_placeholder',  __( 'coupon code', 'wbk' ) ) ) ). '" >';
}
foreach( $payment_methods as $payment_method ){
    ...
    if( $payment_method == 'arrival'  ){
		$button_text = esc_html( get_option( 'wbk_pay_on_arrival_button_text', __( 'Pay on arrival', 'wbk' ) ) );
	...
	if( $payment_method == 'bank' ){
		$button_text = esc_html( get_option( 'wbk_bank_transfer_button_text', __( 'Pay by bank transfer', 'wbk' ) ) );
