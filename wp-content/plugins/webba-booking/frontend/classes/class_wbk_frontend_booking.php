	...
	public function enqueueScripts() {
		...
		if ( wbk_fs()->is__premium_only() ) {
		    if ( wbk_fs()->can_use_premium_code() ) {
				if( get_option( 'wbk_load_stripe_js', 'yes' ) == 'yes' ){
			 		wp_enqueue_script( 'wbk-ext-stripe-js', 'https://js.stripe.com/v3/', array(), null );
				}
			}
		}
		if( $this->has_shortcode( 'webba_booking' ) || $this->has_shortcode( 'webba_email_landing' ) ||  $this->has_shortcode( 'webba_multi_service_booking' ) ) {
			...
			wp_enqueue_script( 'wbk-validator', plugins_url( '../common/wbk-validator.js', dirname( __FILE__ ) ), array( 'jquery', 'jquery-ui-core', 'jquery-effects-core' ), '4.2.25', $in_footer );
		    ...
		    wp_enqueue_script( 'wbk-frontend', plugins_url( 'js/wbk-frontend.js', dirname( __FILE__ ) ), array( 'jquery', 'jquery-ui-core', 'jquery-effects-core' ), '4.2.25', $in_footer );

			if( get_option( 'wbk_pickadate_load', 'yes' ) == 'yes' ){
			    wp_enqueue_script( 'picker', plugins_url( 'js/picker.js', dirname( __FILE__ ) ), array( 'jquery', 'jquery-ui-core', 'jquery-effects-core' ), '4.2.25', $in_footer );
			    wp_enqueue_script( 'picker-date', plugins_url( 'js/picker.date.js', dirname( __FILE__ ) ), array( 'jquery', 'jquery-ui-core', 'jquery-effects-core' ), '4.2.25', $in_footer );
			    wp_enqueue_script( 'picker-legacy', plugins_url( 'js/legacy.js', dirname( __FILE__ ) ), array( 'jquery', 'jquery-ui-core', 'jquery-effects-core' ), '4.2.25', $in_footer );
			}
			if( get_option( 'wbk_date_input', 'popup' ) == 'popup' ){
				wp_enqueue_style( 'picker-default', plugins_url( 'css/default.css', dirname( __FILE__ ) ), array(),  '4.2.25'  );
				wp_enqueue_style( 'picker-default-date', plugins_url( 'css/default.date.css', dirname( __FILE__ ) ), array(), '4.2.25' );
			} elseif( get_option( 'wbk_date_input', 'popup' ) == 'classic' ){
				wp_enqueue_style( 'picker-classic', plugins_url( 'css/classic.css', dirname( __FILE__ ) ), array(), '4.2.25' );
				wp_enqueue_style( 'picker-classic-date', plugins_url( 'css/classic.date.css', dirname( __FILE__ ) ), array(), '4.2.25' );
			}

	 		wp_enqueue_style( 'wbk-frontend-style-custom', plugins_url( 'css/wbk-frontend-custom-style.css', dirname( __FILE__ ) ), array(),  '4.2.25'  );
	 		wp_enqueue_style( 'wbk-frontend-style', plugins_url( 'css/wbk-frontend-default-style.css', dirname( __FILE__ ) ), array(),  '4.2.25' );
