...
class WBK_Backend {
	...
	public function __construct() {
		...
		add_action( 'in_plugin_update_message-webba-booking/webba-booking-lite.php', array( $this, 'prefix_plugin_update_message' ), 10, 2 );
		add_action( 'in_plugin_update_message-webba-booking-lite/webba-booking-lite.php', array( $this, 'prefix_plugin_update_message' ), 10, 2 );
	...
	public function inline_upload_enquene(){
		...
		if ( isset( $_GET[ 'page' ] ) && ( $_GET[ 'page' ] == 'wbk-options' || $_GET[ 'page' ] == 'wbk-schedule' || $_GET[ 'page' ] == 'wbk-gg-calendars'  ||  $_GET[ 'page' ] == 'wbk-forms'   ) ) {
		 	wp_enqueue_style( 'wbk-backend-style', plugins_url( '/css/wbk-backend.css', __FILE__ ), array(), '4.0.73' );
		}
	...
	public function createAdminMenu() {
        ...
        if (  current_user_can('manage_options') || WBK_Validator::checkAccessToSchedule() || WBK_Validator::checkAccessToGgCalendarPage() ){
            if ( !empty($this->components) ){
	        	...
	        	add_menu_page( $root_name, $root_name, 'read', 'wbk-main', array( $this->components['wbk_backend_schedule'], 'render'), plugins_url( 'images/webba-booking.png', __FILE__) );
				...
				add_submenu_page( 'wbk-main', __( 'Services', 'wbk'), __( 'Services', 'wbk'), 'manage_options', 'wbk-services', array( 'WBK_Renderer', 'render_backend_page' ) );
				add_submenu_page( 'wbk-main', __( 'Service categories', 'wbk'), __( 'Service categories', 'wbk'), 'manage_options', 'wbk-service-categories', array( 'WBK_Renderer', 'render_backend_page' ) );
				add_submenu_page( 'wbk-main', __( 'Appointments', 'wbk'), __( 'Appointments', 'wbk'), 'read', 'wbk-appointments', array( 'WBK_Renderer', 'render_backend_page' ) );
				add_submenu_page( 'wbk-main', __( 'Email templates', 'wbk'), __( 'Email templates', 'wbk'), 'manage_options', 'wbk-email-templates', array( 'WBK_Renderer', 'render_backend_page' ) );
				add_submenu_page( 'wbk-main', __( 'Coupons', 'wbk'), __( 'Coupons', 'wbk'), 'read', 'wbk-coupons', array( 'WBK_Renderer', 'render_backend_page' ) );
				add_submenu_page( 'wbk-main', __( 'Pricing rules', 'wbk'), __( 'Pricing rules', 'wbk'), 'read', 'wbk-pricing-rules', array( 'WBK_Renderer', 'render_backend_page' ) );
				if ( wbk_fs()->is__premium_only() ) {
					if ( wbk_fs()->can_use_premium_code() ) {
						add_submenu_page( 'wbk-main', __( 'Google calendars', 'wbk'), __( 'Google calendars', 'wbk'), 'read', 'wbk-gg-calendars', array( 'WBK_Renderer', 'render_backend_page' ) );
