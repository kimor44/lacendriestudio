...
/**
* Plugin Name: Webba Booking (Premium)
...
* @fs_premium_only backend/classes/class_wbk_backend_gg_calendars.php, backend/classes/class_wbk_backend_coupons.php, common/google_api/google-api-2.9.1/, common/google_api/google-api-2.5/, /common/ical/classes/, /common/stripe/stripe-php/, /common/PayPal/composer/, /common/PayPal/paypal/, /common/PayPal/psr/, /vendor/composer/, /vendor/twilio/, /vendor/ics-parser/, /vendor/dompdf/
* @fs_ignore /vendor/plugion/
*/
...
if ( wbk_fs()->is__premium_only() ) {
    if ( is_plugin_active( 'webba-booking-lite' ) && is_plugin_active( 'webba-booking' ) ) {
    ...
    if ( wbk_fs()->is__premium_only() ) {
        ...
        if ( wbk_fs()->can_use_premium_code() ) {
            ...
            if ( !class_exists( 'ICal\\ICal' ) ) {
                include 'vendor/ics-parser/src/ICal/ICal.php';
                include 'vendor/ics-parser/src/ICal/Event.php';
            }
            
            
            if ( version_compare( PHP_VERSION, '7.2.0' ) >= 0 ) {
                include 'vendor/twilio/sdk/src/Twilio/autoload.php';
                include 'includes/third-parties/class_wbk_sms.php';
            } else {
                include 'includes/third-parties/class_wbk_sms_blank.php';
            }
    ...
    function wbk_daily()
    {
        ...
        if ( wbk_fs()->is__premium_only() ) {
            if ( wbk_fs()->can_use_premium_code() ) {
                ...
                if ( get_option( 'wbk_sms_send_reminder', '' ) == 'true' ) {
                    $customer_remider_ids = array();
                    $service_ids = WBK_Model_Utils::get_service_ids();
                    if ( is_array( $service_ids ) && count( $service_ids ) > 0 ) {
                        foreach ( $service_ids as $service_id ) {
                            $appointment_ids = WBK_Db_Utils::getFutureAppointmentsForService( $service_id, get_option( 'wbk_sms_reminder_days', '1' ) );
                            if ( is_array( $appointment_ids ) && count( $appointment_ids ) > 0 ) {
                                foreach ( $appointment_ids as $appointment_id ) {
                                    ...
                                    if ( get_option( 'wbk_multi_booking', 'disabled' ) == 'disabled' ) {
                                        ...
                                        WBK_Sms::send( $appointment_id, get_option( 'wbk_sms_message_reminder' ) );
                    ...
                    if ( count( $customer_remider_ids ) > 0 ) {
                        foreach ( $customer_remider_ids as $appointment_ids ) {
                            ...
                            date_default_timezone_set( get_option( 'wbk_timezone', 'UTC' ) );
                            WBK_Sms::send( $appointment_ids, get_option( 'wbk_sms_message_reminder' ) );
                            date_default_timezone_set( 'UTC' );
                        }
                    }
                }
