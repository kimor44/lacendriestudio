...
class WBK_Db_Utils {
	static function createTables() {
		...
		// custom on/off days
	   	$wpdb->query(
	        "CREATE TABLE IF NOT EXISTS " . get_option( 'wbk_db_prefix', '' ) . "wbk_days_on_off (
	   	...
	   	$wpdb->query(
	        "CREATE TABLE IF NOT EXISTS " . get_option( 'wbk_db_prefix', '' ) . "wbk_locked_time_slots (
	            id int unsigned NOT NULL auto_increment PRIMARY KEY,
	            service_id int unsigned NOT NULL,
	...
	static function getNotAdminUsers() {
		$arr_users = array();
		$arr_temp = get_users( array( 'role__not_in' => array( 'administrator'), 'fields' => 'user_login' ) );
		if ( count( $arr_temp ) > 0 ) {
	 	...
	 	foreach ( $arrIds as $service_id_this ) {
			...
			foreach ( $timeslots_to_lock as $time_slot_start ) {
				...
				if( $intersect == true ) {
					...
					if ( $wpdb->insert( get_option( 'wbk_db_prefix', '' ) . 'wbk_locked_time_slots', array( 'service_id' => $service_id_this, 'time' => $time_slot_start, 'connected_id' => $appointment_id ), array( '%d', '%d', '%d' ) ) === false ){
						echo -1;
						die();
						return;
					}
	...
	static function updatePaymentStatusByIds( $app_ids ){
		foreach( $app_ids as $app_id ){
			...
			$result_pending = $wpdb->update(
							get_option('wbk_db_prefix', '' ) . 'wbk_appointments',
							array( 'prev_status' => 'paid' ),
							array( 'id' => $app_id, 'prev_status' => 'pending' ),
							array( '%s' ),
							array( '%d', '%s' )
						);
			$result_approved = $wpdb->update(
							get_option('wbk_db_prefix', '' ) . 'wbk_appointments',
							array( 'prev_status' => 'paid_approved' ),
							array( 'id' => $app_id, 'prev_status' => 'approved' ),
							array( '%s' ),
							array( '%d', '%s' )
						);
	...
	static function deleteAppointmentByEmailTokenPair( $email, $token ){
		...
		$deleted_count =  $wpdb->delete( get_option('wbk_db_prefix', '' ) . 'wbk_appointments', array( 'email' =>  $email, 'token' => $token ), array( '%s', '%s' ) );
		if ( $deleted_count > 0 ){
			return true;
		} else {
			return false;
		...
		$arrIds = WBK_Db_Utils::getServices();
	 	...
	 	foreach ( $arrIds as $service_id_this) {
		    ...
		    foreach( $appointments as $appointment ){
		    	$start_cur = $appointment->getTime();
		    	$end_cur = $start_cur + $service_this->getDuration() * 60 + $service_this->getInterval() * 60;
		    	if( WBK_Date_Time_Utils::chekRangeIntersect( $start, $end, $start_cur, $end_cur ) == TRUE ){
		    		$total_quantity += $appointment->getQuantity();
		    	}
		    }
	...
	public static function message_placeholder_processing_multi_service( $message, $appointment, $total_amount = null, $current_category = 0, $multi_token = null, $multi_token_admin = null, $app_total_price = null ){
		$service = self::initServiceById( self::getServiceIdByAppointmentId( $appointment->getId() ) );
		return WBK_Db_Utils::message_placeholder_processing( $message, $appointment, $service, $total_amount, $current_category, $multi_token, $multi_token_admin, $app_total_price );
	}
	public static function message_placeholder_processing( $message, $appointment, $service, $total_amount = null, $current_category = 0, $multi_token = null, $multi_token_admin = null, $app_total_price = null ){
		...
		// end total amount
		$price_format = get_option( 'wbk_payment_price_format', '$#price' );
		$tax_amount =  str_replace( '#price', number_format( $tax_amount,  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
		$subtotal_amount =  str_replace( '#price', number_format( $subtotal_amount,  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), '' ), $price_format );
		// beging extra data
		...
		$price_format = get_option( 'wbk_payment_price_format', '$#price' );
		$one_slot_price = str_replace( '#price', number_format( $service->getPrice( $appointment->getTime() ),  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
		...
		if( !is_null( $app_total_price ) ){
			$app_total_price = str_replace( '#price', number_format( $app_total_price,  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
	...
	public static function subject_placeholder_processing( $message, $appointment, $service, $total_amount = null, $current_category = 0  ){
		...
		$message = str_replace( '#invoice_number', get_option( 'wbk_email_current_invoice_number', '1' ), $message );
		$price_format = get_option( 'wbk_payment_price_format', '$#price' );
		$one_slot_price = str_replace( '#price', number_format( $service->getPrice(),  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
		$message = str_replace( '#one_slot_price', $one_slot_price, $message );
		$message = str_replace( '#duration', $service->getDuration(), $message );
		$moment_price = WBK_Db_Utils::getAppointmentMomentPrice( $appointment->getId() );
		$moment_price = str_replace( '#price', number_format( $moment_price,  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
		...
		date_default_timezone_set( get_option( 'wbk_timezone', 'UTC' ) );
 		if( get_option( 'wbk_multi_booking', 'disabled' ) != 'disabled'  ){
			...
			$total_price =  str_replace( '#price', number_format( $total,  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
		 	...
		 	$thanks_message = WBK_Db_Utils::message_placeholder_processing(  $thanks_message, $appointment, $service, $total_price, null, 0, $multi_token, $app_price_total );
 	...
 	static function addAppointmentDataToGGCelendar( $service_id, $appointment_id ){
		if ( wbk_fs()->is__premium_only() ) {
		    if ( wbk_fs()->can_use_premium_code() ) {
		    	global $wpdb;
				$service = self::initServiceById( $service_id );
				$appointment = self::initAppointmentById( $appointment_id );

				$gg_calendars = $service->getGgCalendars();
				if( $gg_calendars == '' ){
					return;
				}
				if( $service->getQuantity() == 1 || get_option( 'wbk_gg_group_service_export', 'event_foreach_appointment' ) == 'event_foreach_appointment' ){
					...
					$add_to_event = '';
					$appointment_ids = self::getAppointmentsByServiceAndTime( $service_id, $appointment->getTime() );
					if( count( $appointment_ids ) > 1 ){
						$title_arr = [];
						$description_arr = [];
						foreach( $appointment_ids as $appointment_id ){
							// additional check to avoid appearance of not paid appointments in the calendar
							if( get_option('wbk_gg_when_add', 'onbooking' ) == 'onpaymentorapproval' ){
								if( get_option('wbk_appointments_default_status', 'approved') == 'approved' ){
									if( self::getAppointmentStatus( $appointment_id ) == 'approved' ){
									    continue;
									}
								}
							}
							...
							$cur_event = $wpdb->get_var( $wpdb->prepare( "SELECT gg_event_id FROM " . get_option( 'wbk_db_prefix', '' ) . "wbk_appointments WHERE id = %d", $appointment_id ) );
		 					if( !is_null( $cur_event ) ){
								if( $cur_event != '' ){
									$add_to_event = $cur_event;
								}
							}
						}
					...
					if( $add_to_event != '' ){
						foreach( $appointment_ids as $appointment_id ){
							self::setGoogleEventsData( $appointment_id, $add_to_event  );
						}
						$event_id_arr = json_decode( $add_to_event );
						foreach( $event_id_arr as $event ){
							$google = new WBK_Google();
					  		$google->init( $event[0] );
					  		$connect_status = $google->connect();
					  		if( $connect_status[0]  == 1 ){
					  			$google->updateEvent( $event[1], $title, $description );
					  		} else {
				...
				// $difference = WBK_Time_Math_Utils::get_offset_difference_with_midnight( $appointment->getTime()  );
			 	$booking_time = $appointment->getTime();

				$start = date( 'Y-m-d', $booking_time ) . 'T' . date(  'H:i:00', $booking_time   );
				$end = date( 'Y-m-d', $booking_time + $service->getDuration() * 60  ) . 'T' . date(  'H:i:00', $booking_time + $service->getDuration() * 60 );
				$gg_calendars = json_decode( $gg_calendars );
	...
	static function addAppointmentDataToCustomerGGCelendar( $service_id, $appointment_ids, $code ){
 	...
 	static function updateAppointmentDataAtGGCelendar( $appointment_id ){
		...
		if( $service->getQuantity() > 1 && get_option( 'wbk_gg_group_service_export', 'event_foreach_appointment' ) == 'one_event'  ){
			if ( wbk_fs()->is__premium_only() ) {
    			if ( wbk_fs()->can_use_premium_code() ) {
					self::deleteAppointmentDataAtGGCelendar( $appointment_id, false );
					self::setGoogleEventsData( $appointment_id, '' );
					self::addAppointmentDataToGGCelendar( $service_id, $appointment_id );
				...
				foreach( $event_id_arr as $event ){
					if( $service->getQuantity() == 1 || get_option( 'wbk_gg_group_service_export', 'event_foreach_appointment' ) == 'event_foreach_appointment'  ){
						...
						if( $by_time ){
							...
							$appointment_ids = $wpdb->get_col( $wpdb->prepare( "SELECT id FROM " . get_option( 'wbk_db_prefix', '' ) . "wbk_appointments WHERE gg_event_id = %s", $event_id_json  ) );
						}
						if( count( $appointment_ids ) > 1 ){
							$title_arr = [];
							$description_arr = [];
							foreach ( $appointment_ids as $appointment_id_cur ){
								if( $appointment_id == $appointment_id_cur ){
									continue;
								}
								// additional check to avoid appearance of not paid appointments in the calendar
								if( get_option('wbk_gg_when_add', 'onbooking' ) == 'onpaymentorapproval' ){
								...
								$title = get_option( 'wbk_gg_calendar_event_title', '#customer_name' );
							  	$description = get_option( 'wbk_gg_calendar_event_description', '#customer_name #customer_phone' );
							  	$description = str_replace( '{n}', "\n",  $description);
								$appointment = self::initAppointmentById( $appointment_id_cur );
								$service = self::initServiceById( $service_id );
							  	$title = self::subject_placeholder_processing_gg( $title, $appointment, $service );
								$description = WBK_Db_Utils::message_placeholder_processing_gg( $description, $appointment, $service );
								$title_arr[] = $title;
	...
	static function getCountOfAppointmentsByEmailTimeService( $email, $time, $service_id ){
		global $wpdb;
		$count = $wpdb->get_var( $wpdb->prepare( " SELECT COUNT(*) FROM " . get_option( 'wbk_db_prefix', '' ) . "wbk_appointments WHERE email=%s and time=%d and service_id=%d", $email, $time, $service_id ) );
	...
	static function getCountOfAppointmentsByEmailServiceDay( $email, $service_id, $day ){
		global $wpdb;
		$count = $wpdb->get_var( $wpdb->prepare( " SELECT COUNT(*) FROM " . get_option( 'wbk_db_prefix', '' ) . "wbk_appointments WHERE email=%s and service_id=%d and day = %d", $email, $service_id, $day ) );
		return $count;
	}
	// get count of appointment by email-service
	...
	static function setIPToAppointment( $appointment_id ){
		...
		$result = $wpdb->update(
						get_option( 'wbk_db_prefix', '' ) . 'wbk_appointments',
						array( 'user_ip' =>   $_SERVER['REMOTE_ADDR'] ),
						array( 'id' => $appointment_id) ,
						array( '%s' ),
						array( '%d' )
					);
	...
	static function setServiceCategoryToAppointment( $appointment_id, $category_id ){
		...
		$result = $wpdb->update(
						get_option( 'wbk_db_prefix', '' ) . 'wbk_appointments',
						array( 'service_category' => $category_id ),
						array( 'id' => $appointment_id) ,
						array( '%d' ),
						array( '%d' )
					);
	...
	static function setAmountForApppointment( $apppointment_id ){
		...
		$amount =   number_format( $price_per_appointment,  get_option( 'wbk_price_fractional', '2' ), '.', ''  );
		$result = $wpdb->update(
					get_option( 'wbk_db_prefix', '' ) . 'wbk_appointments',
	...
	static function getCountOfAppointmentsByDayService( $service_id, $day ){
		...
		$appts_on_day = $wpdb->get_var( $wpdb->prepare( "SELECT count(*) as cnt
														 FROM " . get_option( 'wbk_db_prefix', '' ) . "wbk_appointments
														 where service_id in (" . implode( ',', $arrIds ) . ")  AND day = %d",
														 $day ) );
		return $appts_on_day;
