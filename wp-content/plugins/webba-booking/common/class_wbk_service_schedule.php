...
class WBK_Service_Schedule {
	...
	public function buildSchedule( $day, $ignore_optimization = false, $skip_gg_calendar = false, $ignore_preparation = false, $calculate_availability = false ){
		$sp = new WBK_Schedule_Processor();
		$this->day = $day;
		if( !is_null( $this->gg_breakers ) ){
			$sp->set_gg_breakers( $this->gg_breakers );
		}
		$this->timeslots = $sp->get_time_slots_by_day( $day, $this->service_id, $skip_gg_calendar, $ignore_preparation, $calculate_availability );
	...
	public function renderDayBackend() {
		...
		foreach ( $this->timeslots as $timeslot ) {
			...
			$time_controls = apply_filters( 'wbk_backend_schedule_time_controls', $time_controls, $timeslot, $this->service_id  );
	...
	public function timeSlotStatus( $time, $duration ) {
		...
		if ( wbk_fs()->is__premium_only() ) {
		    if ( wbk_fs()->can_use_premium_code() ) {
		    	 // check gg breakers
				$intersect = false;
				if( ( $this->service->getQuantity( $time ) > 1 && get_option( 'wbk_gg_2way_group', 'lock' ) == 'lock' ) || $this->service->getQuantity( $time ) == 1 ){
	...
	public function renderDayFrontend( $time_after, $offset ){
		...
		if( !WBK_Validator::checkInteger( $night_houts_addon, 1, 64800 ) ){
			...
			$service_schedule_tomorrow->buildSchedule( $tomorrow, false, false, true, true );
	...
	public function fableCount( $time ){
		...
		$service = new WBK_Service_deprecated();
 		...
 		$total_duration= $service->getDuration() * 60 + $service->getInterval() * 60;
  		$booked = $wpdb->get_var(
									$wpdb->prepare(	"SELECT sum(quantity) FROM " . get_option( 'wbk_db_prefix', '' ) . "wbk_appointments WHERE service_id = %d AND  time = %d",
									$this->service_id,
									$time
								)
 								);
		...
		$booked2 = $wpdb->get_var(
									$wpdb->prepare(	"SELECT sum(quantity) FROM " . get_option( 'wbk_db_prefix', '' ) . "wbk_appointments WHERE service_id = %d AND  ( time < %d AND ( time + %d ) > %d)",
	...
	public function getAvailableCount( $time, $ignore_parital = false  ){
		...
		if( get_option( 'wbk_mode_overlapping_availabiliy', 'true' ) == 'true' ){
			...
			$booked = $wpdb->get_var(
				$wpdb->prepare(	"SELECT sum(quantity) FROM " . get_option( 'wbk_db_prefix', '' ) . "wbk_appointments WHERE ( service_id = %d AND  time = %d )",$this->service_id, $time )
			);
		}
 		if ( $booked === NULL ){
 			$booked = 0;
 	...
 	public function	loadFromGGCalendar( $day ){
 		...
 		if ( wbk_fs()->is__premium_only() ) {
		    if ( wbk_fs()->can_use_premium_code() ) {
				...
				$start = date( 'c', $day );
				$end   = date( 'c', $day + 86400 );
				$gg_calendars = $this->service->getGgCalendars();
				if( $gg_calendars == '' ){
					return array();
				}
				...
				foreach ( $gg_calendars as $calendar_id ) {
					...
					if( $connect_status[0]  == 1 ){
						...
						$noifications = new WBK_Email_Notifications( $this->service->getId(), null );
				...
				if( is_array( $add_before_after ) ){
					if( in_array( $this->service_id, $add_before_after ) ){
						$correction = $this->service->getDuration() * 60;
	...
	function intersectWithGoogleEvents( $start, $end ){
		if ( wbk_fs()->is__premium_only() ) {
		    if ( wbk_fs()->can_use_premium_code() ) {
		    	$i = 0;
				if( is_array( $this->gg_breakers ) ){
					foreach( $this->gg_breakers as $gg_slot ){
						$start_compare = $gg_slot->getStart();
						$end_compare = $gg_slot->getEnd();
						if( WBK_Date_Time_Utils::chekRangeIntersect( $start, $end, $start_compare, $end_compare ) ){
							$i++;
						}
					}
				}
...
add_filter( 'wbk_business_hours_for_service', 'webba_native_wbk_business_hours_for_service', 10, 3 );
