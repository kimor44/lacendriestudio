...
class WBK_Ajax_Controller {
	public function __construct() {
		...
		// add action for stripe charge
		if ( wbk_fs()->is__premium_only() ) {
		    if ( wbk_fs()->can_use_premium_code() ) {
				add_action( 'wp_ajax_wbk_stripe_charge', array( $this, 'ajaxChargeStripe') );
				add_action( 'wp_ajax_nopriv_wbk_stripe_charge', array( $this, 'ajaxChargeStripe') );
		    }
		}
	...
	public function ajaxSearchTime() {
		...
		$search_allowed = apply_filters( 'wbk_time_slots_search_allowed', true, $service_id, $date );

		if( $search_allowed !== true ){
			$result =  array( 'dest' => 'slot', 'data'  => $search_allowed );
	...
	public function ajaxBook() {
		...
		foreach ( $times as $time ) {
			...
			if( get_option( 'wbk_appointments_default_status', 'approved' ) == 'approved' ){
				...
				Plugion()->set_value( get_option('wbk_db_prefix', '' ) . 'wbk_appointments', 'appointment_prev_status', $appointment_id, 'approved'  );
			} else {
				WBK_Db_Utils::updateAppointmentStatus( $appointment_id, 'pending' );
				Plugion()->set_value( get_option('wbk_db_prefix', '' ) . 'wbk_appointments', 'appointment_prev_status', $appointment_id, 'pending'  );
			}
			...
			if(	get_option( 'wbk_gg_when_add', 'onbooking' ) == 'onbooking' ){
				if ( wbk_fs()->is__premium_only() ) {
					if ( wbk_fs()->can_use_premium_code() ) {
						...
						WBK_Db_Utils::addAppointmentDataToGGCelendar( $service_id, $booking_id );
					}
				}
			}
		}
		...
		if ( wbk_fs()->is__premium_only() ) {
			if ( wbk_fs()->can_use_premium_code() ) {
				// send sms
				if( get_option( 'wbk_sms_send_on_booking', '' ) == 'true' ){
					if( get_option( 'wbk_multi_booking', 'disabled' ) == 'disabled' ){
						WBK_Sms::send( $appointment_ids[0], get_option( 'wbk_sms_message_on_booking' ) );
					} else {
						WBK_Sms::send( $appointment_ids, get_option( 'wbk_sms_message_on_booking' ) );
					}
				}
			}
		}
		...
		array_multisort( $sort_array, SORT_ASC, SORT_NUMERIC,  $notification_appointment_ids );
		...
		array_multisort( $sort_array, SORT_ASC, SORT_NUMERIC,  $appointment_ids );
			...
			if ( wbk_fs()->is__premium_only() ) {
			    if ( wbk_fs()->can_use_premium_code() ) {
			    	foreach ( $appointment_ids as $app_id ){
						WBK_Db_Utils::setPaymentMethodToAppointment( $app_id, '' );
					}
					$paypal = new WBK_PayPal();
					if ( $paypal->init( $referer[0], $appointment_ids ) === FALSE ){
						echo 'Unable to initalize PayPal API.';
						date_default_timezone_set('UTC');
						wp_die();
						return;
					}
					$html = $paypal->createPayment( $method, $appointment_ids, $coupon_result );
					if ( $html == -9 ){
						wbk_set_appointment_as_paid_with_coupon( $appointment_ids, $method );
					}
					if ( $html == -1 ){
					...
					if ( $html == -5 ){
						echo  __( 'Unable to create payment.', 'wbk' );
						date_default_timezone_set('UTC');
						wp_die();
						return;
					}
		...
		if( $method == 'stripe' ){
			if ( wbk_fs()->is__premium_only() ) {
			    if ( wbk_fs()->can_use_premium_code() ) {
					...
					if( $stripe->init( WBK_Db_Utils::getServiceIdByAppointmentId( $app_id ) ) == TRUE ){

						$html  = $stripe->createPayment( $method, $appointment_ids,  $coupon_result );

					 	$html .= '<div class="wbk_mt_25 wbk-clear-block"></div>';
					 	$html .= '<hr class="wbk-form-separator"><div id="card-element"></div>';
					 	$approve_btn = WBK_Validator::alfa_numeric( get_option( 'wbk_payment_approve_text', '' ) );

						$html .='<div id="card-errors"></div>';
						$safe_value = $stripe->getOrderData( $app_ids, $coupon_result  );
						...
						foreach( $additional_fields as $key ) {
							$html .= '<label class="wbk-input-label">' . $payment_fields[$key] . '</label>';
							if( $key != 'country' ){
							$html .= '<input name="wbk-' . $key . '"  data-field="' . $key .'" class="wbk-text wbk-input wbk-stripe-additional-field" id="wbk-' . $key . '" autocomplete="false"   type="text">';
							} else {
								$countries = array (
									'AF' => 'Afghanistan',
									'AX' => 'Aland Islands',
								...
								$html .= '<select name="wbk-' . $key . '"  data-field="' . $key .'" class="wbk-text wbk-input wbk-stripe-additional-field" id="wbk-' . $key . '" autocomplete="false">';
		                ...
		                $html .= '<input type="button" disabled data-amount="'. round( $safe_value[0] ) .'" data-app-id="' . implode( ',', $app_ids ) . '" class="wbk-button wbk-width-100 wbk-mt-10-mb-10 wbk-stripe-approval-button" value="' . $approve_btn . '">';
					} else {
						$html = '-1';
					}
					...
					if ( $appointment->load() ) {
						...
						if( $service != FALSE ){
							...
							if ( $super_admin_email != '' ) {
								$notifications->sendMultipleNotification( $appointment_ids, $message, $subject, $super_admin_email  );
							}
						}
					}
				}
	...
	public function ajaxChargeStripe(){
		if ( wbk_fs()->is__premium_only() ) {
		    if ( wbk_fs()->can_use_premium_code() ) {
				...
				$app_ids = explode( ',', sanitize_text_field( $_POST['app_ids'] ) );
				if( count( $app_ids ) == 0 ){
					$error_message = str_replace( '#response',  __( 'appointment ids not provided', 'wbk' )  , $error_message );
					echo( json_encode( array( 0, $error_message ) ) );
					date_default_timezone_set('UTC');
					wp_die();
					return;
				}
				...
				$stripe = new WBK_Stripe();
				if( $stripe->init( WBK_Db_Utils::getServiceIdByAppointmentId( $app_ids[0] ) ) == FALSE ){
					$error_message = str_replace( '#response',  __( 'unable to initalize Stripe object', 'wbk' )  , $error_message );
					echo( json_encode( array( 0, $error_message ) ) );
					date_default_timezone_set('UTC');
					wp_die();
					return;
				...
				if( !is_null( $payment_id) ){
					...
					if( !isset(  $payment_id  ) || $payment_id == '' ){
						$error_message = str_replace( '#response',  __( 'invalid payment id', 'wbk' ), $error_message );
						echo( json_encode( array( 0, $error_message ) ) );
						date_default_timezone_set('UTC');
						wp_die();
						return;
					...
					// validate app ids
					if( !is_array( $app_ids ) ){
						$error_message = str_replace( '#response', __( 'invalid appointment id(s)', 'wbk' ) , $error_message );
						echo( json_encode( array( 0, $error_message ) ) );
						date_default_timezone_set('UTC');
						wp_die();
						return;
					...
					$safe_value = $stripe->getOrderData( $app_ids, $coupon );
					$calulated_amount = $safe_value[0];
					if( ( round( $calulated_amount )  <> round( $amount )  ) || !WBK_Validator::checkInteger( $amount, 0, 10000000 ) ){
						$error_message = str_replace( '#response',  __( 'incorrect amount', 'wbk' ), $error_message );
						echo( json_encode( array( 0, $error_message ) ) );
						date_default_timezone_set('UTC');
						wp_die();
						return;
				...
				// sending payment notifications and google calenadrs
				if( $result[0] == 1  && count( $app_ids ) > 0 ){
			...
			if ( $service->getQuantity() > 1 ) {
				...
				if( is_array( $time ) ){
					...
					foreach ( $time as $time_this ) {
						...
						$sp->get_time_slots_by_day( $day, $service_id, false, true, true );
...
function wbk_set_appointment_as_paid_with_coupon( $app_ids, $method ){
	...
	$html .= '</div>';
	if( $method == 'woocommerce' ){
		echo json_encode( array( 'status' => 5, 'details' => $html ) );
		date_default_timezone_set( 'UTC' );
		wp_die();
		return;
	}
