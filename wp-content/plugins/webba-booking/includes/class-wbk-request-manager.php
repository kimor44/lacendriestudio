...
class WBK_Request_Manager {
    ...
    public function get_available_time_slots_day( $request ) {
        ...
        if( !WBK_Validator::is_service_exists( $service_id ) ){
            $data = array( 'Reason' => 'Service not exists' );
            $response = new \WP_REST_Response( $data );
            $response->set_status( 400 );
            return $response;
        }
        if( !WBK_Validator::is_date( $day ) ){
            $data = array( 'Reason' => 'Wrong date passed' );
            $response = new \WP_REST_Response( $data );
            $response->set_status( 400 );
            return $response;
        }
        ...
        $day = strtotime($day);
        $timeslots = $sp->get_time_slots_by_day( $day, $service_id, false, true, true, $current_booking );
        ...
        */
        ...
        $data = array( 'time_slots' => $timeslots_filtered );
        $response = new \WP_REST_Response( $data  );
    ...
    public function get_wp_users( $request ){
        $data = array( 'none_admin_users' => WBK_User_Utils::get_none_admin_wp_users() );
        $response = new \WP_REST_Response( $data  );
        $response->set_status( 200 );
        return $response;
    }
    ...
    public function wbk_csv_export( $request ){
        if ( wbk_fs()->is__premium_only() ) {
            if ( wbk_fs()->can_use_premium_code() ) {
                $path =  __DIR__ . DIRECTORY_SEPARATOR . '..'.DIRECTORY_SEPARATOR . 'backend' . DIRECTORY_SEPARATOR . 'export' . DIRECTORY_SEPARATOR;
                // new method start
                global $wpdb;
                $table = $wpdb->prefix . 'wbk_appointments';
                ...
                $all_columns = WBK_Model_Utils::get_bookings_page_columns();
                $current_columns = get_option('wbk_appointments_table_columns');
                $file_data = array();
                $row_data = array();
                $fields =  Plugion()->tables->get_element_at( $table )->get_data( 'fields_to_view' );
                $row_data[] = __( 'Booking ID', 'wbk' );
                foreach( $fields as $slug => $field ){
                    if( !$field->get_in_row() ){
                        continue;
                    }
                    if( $slug == 'appointment_extra' ){
                        $custom_fields_list = WBK_Model_Utils::get_custom_fields_list();
                        if( count( $custom_fields_list ) == 0 ){
                            $row_data[] = $field->get_title();
                        } else {
                            foreach( $custom_fields_list as $field_id => $field_title ){
                                $row_data[]  = $field_title;
                            }
                        }
                    } else {
                        if( $slug == 'id' ){
                ...
                $file_data[] = implode( $delimiter, $row_data )  ."\r\n";
                foreach ( Plugion()->tables->get_element_at( $table )->get_data( 'rows' ) as $row ) {
                     ...
                     foreach( $fields as $slug => $field ){
                         ...
                         if( $slug == 'appointment_extra' ){
                             ...
                             if( !is_null( $value ) ){
                                 $extra_row_value = '';
                                 $extra_arr_value = array();
                                 foreach( $value as $item ){
                                     if( is_array( $item ) && count( $item ) == 3 && trim( $item[2] ) != '' ){
                                         $extra_row_value .= $item[1] .': ' . $item[2] .'. ';
                                         $extra_arr_value[ $item[0] ] = $item[2];
                                     }
                                 }
                             }
                             $custom_fields_list = WBK_Model_Utils::get_custom_fields_list();
                             if( count( $custom_fields_list ) == 0 ){
                                 ...
                                 foreach( $custom_fields_list as $custom_id => $custom_title ){
                                     if( isset( $extra_arr_value[ $custom_id] ) ){
                                         $row_data[] = $extra_arr_value[ $custom_id];
                                     } else {
                                         $row_data[] = '';
                                     }
                                 }
                             ...
                             ob_start();
                             do_action( 'plugion_table_cell_' . $field->get_type(), [ $field, $slug, $value, $row ] );
                ...
                $data = array( 'url' => plugins_url( 'webba-booking' ) . '/backend/export/' . $filename );
                $response = new \WP_REST_Response( $data  );
                $response->set_status( 200 );
                return $response;
            }
    ...
    public function calculate_amounts(){
        ...
        $sub_total_formated = str_replace( '#price', number_format( $sub_total,  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
        $tax_amount = WBK_Price_Processor::get_tax_amount( $sub_total, WBK_Price_Processor::get_tax_for_messages() );
        $tax_amount_formated = str_replace( '#price', number_format( $tax_amount,  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
        ...
        $total_formated = str_replace( '#price', number_format( $total_amount,  get_option( 'wbk_price_fractional', '2' ), get_option( 'wbk_price_separator', '.' ), ''  ), $price_format );
        date_default_timezone_set( 'UTC');
        $result = array( 'sub_total' => $sub_total,
                         'tax' => $tax_amount,
                         'total' => $total_amount,
		...
		if( !is_array( $service_id ) ){
            ...
            // set number of days to show - $output_count
	 		if ( get_option( 'wbk_mode', 'extended' ) == 'extended' ) {
				$days_count_opt = apply_filters( 'wbk_days_extended_mode', get_option( 'wbk_days_in_extended_mode', 'default' ), $days_count_opt, $service_id  );
	 		...
	 		while ( $i <= $output_count ) {
                ...
                if ( $day_status == 1 ) {
					if ( get_option( 'wbk_mode', 'extended' ) == 'extended' ) {
	 				...
	 				$timeslots = $sp->get_time_slots_by_day( $day_to_render, $service_id, false, false, true );
                    $html =   WBK_Renderer::load_template( 'frontend/day_with_timeslots', array( $day_to_render, $time_slots, $service_id ) );

                    echo json_encode( array( 'data' => $html, 'dest' => 'slot' ) );
                    wp_die();
                    return;
