...
add_action( 'plugion_before_table', 'my_plugion_before_table', 10, 1 );
function my_plugion_before_table( $slug ){
    ...
    ?>
        <script>
            var wbk_custom_fields = '<?php echo get_option( 'wbk_custom_fields_columns' ); ?>';
        </script>
    <?php
}
...
add_filter( 'plugion_formated_row_values', 'wbk_plugion_formated_row_values', 10, 2 );
...
add_filter( 'plugion_filter_value', 'wbk_plugion_filter_value', 10, 2 );
...
function wbk_plugion_on_after_row_add( $table_name, $table_name_not_filtered, $row ){
    if( $table_name == get_option( 'wbk_db_prefix', '' ) . 'wbk_appointments' ){
        ...
        Plugion()->set_value( get_option( 'wbk_db_prefix', '' ) . 'wbk_appointments', 'appointment_created_on', $row->id, time() );
        Plugion()->set_value( get_option( 'wbk_db_prefix', '' ) . 'wbk_appointments', 'appointment_duration', $row->id, $service->get_duration() );
        Plugion()->set_value( get_option( 'wbk_db_prefix', '' ) . 'wbk_appointments', 'appointment_prev_status', $row->id, $row->status );

        if( get_option( 'wbk_gdrp', 'disabled' ) == 'disabled' ){
            if( isset( $_SERVER['REMOTE_ADDR'] ) ){
                Plugion()->set_value( get_option( 'wbk_db_prefix', '' ) . 'wbk_appointments', 'appointment_user_ip', $row->id, $_SERVER['REMOTE_ADDR'] );
            }
        }
        ...
        if ( wbk_fs()->is__premium_only() ) {
            if ( wbk_fs()->can_use_premium_code() ) {
                if( get_option( 'wbk_sms_send_on_manual_booking', '' ) == 'true' ){
                    WBK_Sms::send( $row->id, get_option( 'wbk_sms_message_on_booking' ) );
                }
                     ...
                     if ( wbk_fs()->can_use_premium_code() ) {
                         ...
                         if( get_option( 'wbk_sms_send_on_approval', '' ) == 'true' ){
                             ...
                             if( get_option( 'wbk_multi_booking', 'disabled' ) == 'disabled' ){
                                 WBK_Sms::send( $row->id, get_option( 'wbk_sms_message_on_approval' ) );
                             } else {
                                 WBK_Sms::send( array( $row->id ), get_option( 'wbk_sms_message_on_approval' ) );
                             }
                             date_default_timezone_set( 'UTC' );
                         }
                     }
    ...
    if ( wbk_fs()->is__premium_only() ) {
        if ( wbk_fs()->can_use_premium_code() ) {
            if( $slug == get_option( 'wbk_db_prefix', '' ) . 'wbk_appointments' || $slug == get_option( 'wbk_db_prefix', '' ) . 'wbk_cancelled_appointments' ){
                ...
                echo '<a id="wbk_csv_export" style="float:left" data-delimiter="' . $delimiter  . '" class="plugion_button plugion_transparent_dark_button">' . __( 'Export to csv file', 'wbk' ) . '</a>';
                echo '<div class="plugion_loader plugion_loader_quad plugion_hidden" style="float:left;"></div>';
            }
        }
    }
