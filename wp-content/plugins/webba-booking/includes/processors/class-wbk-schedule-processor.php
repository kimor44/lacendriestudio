...
class WBK_Schedule_Processor {
    ...
    public function get_time_slots_by_day( $day,
                                           $service_id,
        ...
        if (!$skip_gg_calendar) {
            if (wbk_fs()->is__premium_only()) {
                if (wbk_fs()->can_use_premium_code() ) {
                    ...
                    $this->gg_breakers = $this->load_gg_events_in_range( $start_all, $end_all, $service );
                    // load data from external ics file
                    if( $service->get('extcalendar') != '' ){
                        if ( !$skip_gg_calendar ) {
                            $extcalendars = explode( ',', $service->get('extcalendar') );
                            foreach( $extcalendars as $extcalendar ){
                                try {
                                    $ical = new ICal\ICal();
                                    $ical->initUrl( $extcalendar );
                                    foreach( $ical->events() as $event ){
                                        $dtstart = $ical->iCalDateToDateTime( $event->dtstart_array[3] );
                                        $dtsend = $ical->iCalDateToDateTime( $event->dtend_array[3] );
                                        $breaker = new WBK_Time_Slot( $dtstart->getTimestamp(), $dtsend->getTimestamp() );
                                        $this->ext_breakers[] = $breaker;
                                    }
                                } catch ( \Exception $e ){
                                    continue;
                                }
        ...
        for ($i = 0; $i < count( $timeslots ); $i++) {
                ...
                if( $timeslots[$i]->getStatus() == 0 || is_array( $timeslots[$i]->getStatus() ) ){
                    ...
                    if( count(  $connected_service_ids ) > 0 ){
                        ...
                        foreach( $booking_ids as $booking_id ){
                            $booking = new WBK_Booking( $booking_id );
                            if( WBK_Time_Math_Utils::check_range_intersect( $timeslots[$i]->getStart(), $timeslots[$i]->getEnd(), $booking->get_start(), $booking->get_full_end() ) ){
                    ...
                    if( get_option( 'wbk_mode_overlapping_availabiliy', 'true' ) == 'true' ){
                        ...
                        foreach( $booking_ids as $booking_id ){
                            ...
                            if( $timeslots[$i]->getStart() != $booking->get_start() ){
                                ...
                                if( WBK_Time_Math_Utils::check_range_intersect( $timeslots[$i]->getStart(), $timeslots[$i]->getEnd(), $booking->get_start(), $booking->get_end() ) ){
                    ...
                    if ( wbk_fs()->is__premium_only() ) {
                        if ( wbk_fs()->can_use_premium_code() ) {
                            foreach( $this->gg_breakers as $gg_slot ){
                                $start_compare = $gg_slot->getStart();
                                $end_compare = $gg_slot->getEnd();
                                if( WBK_Time_Math_Utils::check_range_intersect( $timeslots[$i]->getStart(), $timeslots[$i]->getEnd(), $start_compare, $end_compare ) ){
                                    $gg_count++;
                                }
                            }
                            foreach( $this->ext_breakers as $ext_slot ){
                                $start_compare = $ext_slot->getStart();
                                $end_compare = $ext_slot->getEnd();
                                if( WBK_Time_Math_Utils::check_range_intersect( $timeslots[$i]->getStart(), $timeslots[$i]->getEnd(), $start_compare, $end_compare ) ){
                                    $ext_count++;
                                }
                            }
                    ...
                    $available = apply_filters( 'wbk_available_at_time', $available, $timeslots[$i]->getStart(), $day, $service_id );
            ...
            if ($timeslot_time_string == 'start_end') {
                    ...
                    $time = wp_date($time_format, $timeslots[$i]->getStart(), $timezone_to_use) . ' - ' .
                            wp_date($time_format, $end_minus_gap, $timezone_to_use_end);
            ...
            if ($timeslot_time_string_backend == 'start') {
                ...
                $time = wp_date($time_format, $timeslots[$i]->getStart(), $timezone_to_use) . ' - ' .
                        wp_date($time_format, $end_minus_gap, $timezone_to_use);
            }
            $timeslots[$i]->set_formated_time_backend($time);
        }

        $timeslots = apply_filters( 'get_time_slots_by_day', $timeslots, $day, $service_id );
    ...
    public function load_gg_events_in_range( $start, $end, $service ){
        ...
        if (wbk_fs()->is__premium_only()) {
            if (wbk_fs()->can_use_premium_code()) {
                ...
                $start = date('c', $start );
                $end   = date('c', $end );
                $gg_calendars = $service->get_gg_calendars();
                if ($gg_calendars == '') {
                    return [];
                }
                ...
                foreach ($gg_calendars as $calendar_id) {
                    ...
                    $init_result = $google->init($calendar_id);
                    if ($init_result == false) {
                        continue;
                    }
                    if ($google->getCalendarMode() == 'One-way') {
                        continue;
                    }
                    $connect_status = $google->connect();
                    if( $connect_status[0]  == 1) {
                        $gg_breaked = $google->getEventsTimeRanges($start, $end);
                        if ($gg_breaked != false) {
                            $event_data_arr = array_merge($event_data_arr, $gg_breaked);
                        }
                    } else {
                        $noifications = new WBK_Email_Notifications( $service->get_id(), null );
						$noifications->send_gg_calendar_issue_alert_to_admin( $connect_status[1] );
                    }
                }
    ...
    public function get_business_hours_intervals_by_dow($dow, $service_id) {
        ...
        usort( $slots, function($a, $b) { return (int)( $a->start > $b->start ); } );
        return $slots;
    }
    ...
    public function get_time_slot_status($time, $duration, $service) {
        ...
        if (wbk_fs()->is__premium_only()) {
            if (wbk_fs()->can_use_premium_code()) {
                // check gg breakers
                $intersect = false;
                if ( ( $service->get_quantity() > 1 && get_option('wbk_gg_2way_group', 'lock') == 'lock') || $service->get_quantity() == 1) {
                    foreach ($this->gg_breakers as $breaker) {
                        $cur_start = $breaker->getStart();
                        $cur_end = $breaker->getEnd();
                        if( WBK_Time_Math_Utils::check_range_intersect( $start, $end,$cur_start, $cur_end ) == true ){
                            return -2;
                        }
                    }
                }
                // check ext breakers
                if ( ( $service->get_quantity() > 1 && $service->get('extcalendar_group_mode') == 'lock') || $service->get_quantity() == 1) {
                    foreach( $this->ext_breakers as $breaker ) {
                        $cur_start = $breaker->getStart();
                        $cur_end = $breaker->getEnd();
                        if( WBK_Time_Math_Utils::check_range_intersect( $start, $end,$cur_start, $cur_end ) == true ){
                            return -2;
